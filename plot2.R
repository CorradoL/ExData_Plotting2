...
BCtotPM25 <- NEI %>%                                     # from the NEI database
    ...
    summarise(total = sum(Emissions)) %>%   # sum of Emissions each group (year)
...
def.bars <- barplot(  # create a barplot (and save to store x-coord of the bars)
    height = BCtotPM25$total,   # of the total PM2.5 each year in Baltimora City
    density = 75,                              # with a partial filling of color
    angle = -20,                      # quite descending angle for texture lines
    names.arg = BCtotPM25$year,                       # bars' names are the year
    col = 'steelblue4',                           # my idea of pollution's color
    main = 'Total pm2.5 in Baltimora city (1999 - 2008)',       # with its title
    xlab = 'years',                                           # and axes' labels
    ylab = 'pm 2.5 (tons)'
)
lines(x  = def.bars, y = BCtotPM25$total/2)      # baricentric line to emphasize
